defmodule Watermelon.Banking.TellerServiceImplementation do
  ...
  @impl true
  def account_by_id(token, account_id) do
    {:ok, prepare_data_for_show_account(token, account_id)}
  end
  ...
  defp prepare_data_for_show_account(token, account_number) do
    accounts =
      data()
      |> Keyword.get(String.to_atom(token))

    accounts
     |> Enum.find( fn(acc) -> acc[:account_number] == account_number end)
  end
